...
const ohMy = () => {
    axios.get("http://localhost:3000/animals").then((res) => {
        for(i = 0; i < res.data.length; i++) {
            ...
            edit.textContent = res.data[i]
            let bodyDouble = document.querySelector(`body`)
            bodyDouble.appendChild(edit)
        }
    })
}
...
const repeatMyParam = () => {
    axios.get("http://localhost:3000/repeat/test").then((res) => {
...
const queryTester = () => {
    axios.get(`http://localhost:3000/query-test?bayonetta=1&2&3`).then((res)=> 
    console.log(res.data))
}

document.getElementById(`query-button`).addEventListener(`click`, queryTester)
////////////////
//INTERMEDIATE//
////////////////
...
/*
    ...
    It prints out a message saying you sent an empty query
    2: Send more than 1 query on the URL -- what happened? 
    It printed out a message saying You sent more than one query! along with an array of the different variables I strung to the query
*/
